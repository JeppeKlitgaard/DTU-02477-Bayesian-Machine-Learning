...
/// Common function for vectors and matrices
///
/// ```example
/// #let vv = linalg-notation.with(underbars: 0, bold: true, upright: false)
/// ```
#let linalg-notation(underbars: 0, bold: true, upright: false, body) = {
  for _ in range(underbars) {
    body = underbar(body)
  }
  ...
  if upright {
    body = math.upright(body)
  }

  body
}

#let vector = linalg-notation
#let matrix = linalg-notation